...
class FeatureEngineering:
    ...
    def _add_technical_indicators(self, df: pd.DataFrame) -> None:
        ...
        if 'trend' in ta_feature_sets:
            ...
            for period in lookback_periods:
                ...
                df[f'close_to_sma_{period}'] = close_prices / df[f'sma_{period}']
            ...
            for period in lookback_periods:
                ...
                df[f'close_to_ema_{period}'] = close_prices / df[f'ema_{period}']
            ...
            df['sar_ratio'] = close_prices / df['sar']
            
            # Average Directional Index (ADX) - Trend strength
            df['adx'] = talib.ADX(high_prices, low_prices, close_prices, timeperiod=14)
            
            # Ichimoku Cloud
            df['tenkan_sen'] = (pd.Series(high_prices).rolling(window=9).max() + 
                               pd.Series(low_prices).rolling(window=9).min()) / 2
            df['kijun_sen'] = (pd.Series(high_prices).rolling(window=26).max() + 
                              pd.Series(low_prices).rolling(window=26).min()) / 2
            df['senkou_span_a'] = ((df['tenkan_sen'] + df['kijun_sen']) / 2).shift(26)
            df['senkou_span_b'] = ((pd.Series(high_prices).rolling(window=52).max() + 
                                  pd.Series(low_prices).rolling(window=52).min()) / 2).shift(26)
            df['chikou_span'] = pd.Series(close_prices).shift(-26)
        ...
        if 'volatility' in ta_feature_sets:
            ...
            for period in [20]:  # Standard Bollinger Band period
                ...
                df[f'bb_upper_{period}'] = upperband
                df[f'bb_middle_{period}'] = middleband
                df[f'bb_lower_{period}'] = lowerband
                df[f'bb_width_{period}'] = (upperband - lowerband) / middleband
                df[f'bb_position_{period}'] = (close_prices - lowerband) / (upperband - lowerband)
            
            # Normalized historical volatility
            for period in lookback_periods:
                returns = pd.Series(close_prices).pct_change(1)
                df[f'volatility_{period}'] = returns.rolling(window=period).std() * np.sqrt(252)
            
            # Keltner Channels
            for period in [20]:
                df[f'keltner_middle_{period}'] = talib.SMA(close_prices, timeperiod=period)
                atr = talib.ATR(high_prices, low_prices, close_prices, timeperiod=period)
                df[f'keltner_upper_{period}'] = df[f'keltner_middle_{period}'] + 2 * atr
                df[f'keltner_lower_{period}'] = df[f'keltner_middle_{period}'] - 2 * atr
        
        # Add volume indicators
        if 'volume' in ta_feature_sets and volume is not None:
            # On-Balance Volume (OBV)
            df['obv'] = talib.OBV(close_prices, volume)
            
            # Volume Oscillator
            df['short_vol_ma'] = pd.Series(volume).rolling(window=5).mean()
            df['long_vol_ma'] = pd.Series(volume).rolling(window=10).mean()
            ...
            df['cmf'] = pd.Series(mf_volume).rolling(window=20).sum() / pd.Series(volume).rolling(window=20).sum()
            
            # Ease of Movement
            move = ((high_prices + low_prices) / 2) - ((high_prices.shift(1) + low_prices.shift(1)) / 2)
            box_ratio = (volume / 1000000) / (high_prices - low_prices)
            df['eom'] = move / box_ratio
            df['eom_ma'] = pd.Series(df['eom']).rolling(window=14).mean()
            
            # Price-Volume Trend
            df['pvt'] = (close_prices.pct_change() * volume).cumsum()
    ...
    def _add_price_patterns(self, df: pd.DataFrame) -> None:
        ...
        df['cdl_morning_star'] = talib.CDLMORNINGSTAR(open_prices, high_prices, low_prices, close_prices)
        df['cdl_evening_star'] = talib.CDLEVENINGSTAR(open_prices, high_prices, low_prices, close_prices)
        df['cdl_shooting_star'] = talib.CDLSHOOTINGSTAR(open_prices, high_prices, low_prices, close_prices)
        df['cdl_harami'] = talib.CDLHARAMI(open_prices, high_prices, low_prices, close_prices)
        
        # Convert binary/ternary patterns to -1, 0, 1 scale for easier use in ML models
        ...
        for window in [5, 10, 21]:
            if 'close' in result_df.columns:
                ...
                if ma_col not in result_df.columns:
                    result_df[ma_col] = talib.SMA(result_df['close'].values, timeperiod=window)
