...
A professional-grade bull call spread implementation that leverages the modular,
event-driven architecture. This strategy is designed to profit from moderately
bullish market movements with defined risk and reward.
"""
...
@register_strategy(
    ...
    market_type="options",
    description="A strategy that buys a call option at a lower strike price and sells another call at a higher strike, profiting from moderately bullish movement",
    timeframes=["1d", "1w"],
    parameters={
        "delta_lower_long": {"description": "Target delta for long call leg (lower bound)", "type": "float"},
        "delta_upper_long": {"description": "Target delta for long call leg (upper bound)", "type": "float"},
        "preferred_width_pct": {"description": "Preferred width between strikes as % of stock price", "type": "float"},
        "min_bullish_score": {"description": "Minimum bullish score to enter (0-100)", "type": "float"}
    }
)
class BullCallSpreadStrategy(VerticalSpreadEngine):
    """
    Bull Call Spread Strategy
    
    This strategy buys a call option at a lower strike price and sells another 
    call option at a higher strike price with the same expiration. It profits 
    when the underlying security price rises moderately.
    
    Features:
    - Adapts the legacy Bull Call Spread implementation to the new architecture
    ...
    def __init__(self, session: OptionsSession, data_pipeline: DataPipeline, 
        ...
        default_params = {
            ...
            # Bull Call specific parameters
            'delta_lower_long': 0.45,  # Target delta for long call leg
            'delta_upper_long': 0.60,  # Target delta for long call leg
            'delta_lower_short': 0.20, # Target delta for short call leg
            'delta_upper_short': 0.35, # Target delta for short call leg
            
            # Market condition filters
            'min_bullish_score': 60,   # Minimum bullish score to enter (0-100)
        ...
        # Update with default parameters for Bull Call Spreads 
        for key, value in default_params.items():
            if key not in self.parameters:
                self.parameters[key] = value
        
        # Set the spread type for this strategy
        self.spread_type = VerticalSpreadType.BULL_CALL
        
        # Register for market events
        self.register_events()
        
        logger.info(f"Initialized Bull Call Spread Strategy for {session.symbol}")
    
    def calculate_indicators(self, data: pd.DataFrame) -> Dict[str, Any]:
        """
        Calculate technical indicators for the Bull Call Spread strategy.
    ...
    def generate_signals(self, data: pd.DataFrame, indicators: Dict[str, Any]) -> Dict[str, Any]:
        """
        Generate trading signals for Bull Call Spreads based on market conditions.
            ...
            if 'bullish_score' in indicators:
                ...
                if bullish_score >= min_score:
                    ...
                    if rsi_check_passed and macd_check_passed:
                        ...
                        logger.info(f"Bull Call Spread entry signal: score {bullish_score}, strength {signals['signal_strength']:.2f}")
    ...
    def filter_option_chains(self, option_chain: pd.DataFrame) -> pd.DataFrame:
        """
        Apply custom filters for Bull Call Spread option selection.
        ...
        # Additional filters specific to Bull Call Spreads
        
        # Delta filter for selecting appropriate strikes
        if 'delta' in filtered_chain.columns:
            ...
            filtered_chain = filtered_chain[
                ((filtered_chain['option_type'] == 'call') & 
                 ((filtered_chain['delta'] >= self.parameters['delta_lower_long']) & 
                  (filtered_chain['delta'] <= self.parameters['delta_upper_long']))) |
                ((filtered_chain['option_type'] == 'call') & 
                 ((filtered_chain['delta'] >= self.parameters['delta_lower_short']) & 
                  (filtered_chain['delta'] <= self.parameters['delta_upper_short'])))
            ]
    ...
    def _execute_signals(self):
        """
        Execute Bull Call Spread specific trading signals.
        """
        # Use base class implementation for standard vertical spread execution
        super()._execute_signals()
    
    def register_events(self):
        """Register for events relevant to Bull Call Spreads."""
        # Register for common event types from the parent class
        super().register_events()
        
        # Add any Bull Call Spread specific event subscriptions
    ...
    def on_event(self, event: Event):
        """
        Process incoming events for the Bull Call Spread strategy.
        ...
        # Add additional Bull Call Spread specific event handling
